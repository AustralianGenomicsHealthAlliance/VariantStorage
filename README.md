...
To authenticate using the curl command:
    ...
    curl -XPOST -H 'Content-Type:application/json' --data '{"username":"admin", "password":"agha123"}' http://localhost:8080/api/login

As a response from the authentication you will get some tokens in the JSON response. The important token is the 'access_token':
